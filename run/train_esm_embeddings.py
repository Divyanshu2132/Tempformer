...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Tempformer on ESM single/pairwise embeddings")
    parser.add_argument("--data-dir", default="./data", help="Directory of mdcath HDF5 dataset files")
    parser.add_argument("--single-embedding-path", default="./embeddings/esm/mdcath", help="Directory of ESM single embedding .npy files")
    parser.add_argument("--embedding-path", default="./embeddings/esm/mdcath", help="Directory of ESM pairwise embedding .npy files")
...
single_embidding_path = args.single_embedding_path
...
def find_embed_paths(domain_id: str):
    ...
    single_files = os.listdir(single_embidding_path)
    single = [x for x in single_files if domain_id in x and "single" in x and x.endswith(".npy")]
    pair   = [x for x in files if domain_id in x and "pairwise" in x and x.endswith(".npy")]
    if len(single) == 0 or len(pair) == 0:
        return None, None
    return os.path.join(single_embidding_path, single[0]), os.path.join(embidding_path, pair[0])
...
model = MainModel(input_shape=1280).to(device)
